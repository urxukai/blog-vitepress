...
```
1. let不存在变量提升
2. let不允许重复声明（在当前上下文中，不论基于什么方式声明了这个变量，再次基于let、const声明都会报重复声明的错误）
3. 全局上下文中，基于var声明的变量不是存放到VO(G)中的，而是直接放在了GO(window)中；基于let声明的变量是存放在VO(G)中的
4. 暂时性死区
5. 块级作用域
```
...
```
1. 词法解析（AST）：我们基于HTTP从服务器拉取回来的JS代码其实是一些字符串，浏览器首先会按照 ECMAScript 规则，把字符串变为C++可以识别和解析的一套树结构对象
2. 全局上下文
3. 变量提升
4. 代码执行
```

> 没有 ES6(let)之前，作用域只有两种：全局上下文、函数执行的私有上下文；但是有了 let 之后，产生了第三种：块级作用域「如果在{}（排除函数和对象的大括号）中出现 let/const/function，则当前的{}会成为一个块级私有的上下文」
